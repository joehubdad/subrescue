...
func calc_pull():
	...
	var direction2 = $Hose.get_child(0).get_node("PinJoint2D").global_position.direction_to(newp2pos)
	var force2 = direction * $Hose.get_child(0).get_node("PinJoint2D").global_position.distance_to(newp2pos)
	var relevant2 = find_relevant(false)
	
	for i in $Hose.get_children():
		var p := Vector2.ZERO
		
		var next = null
		var next_idx = i.get_index() + 1
		if next_idx < $Hose.get_children().size():
			next = $Hose.get_child(next_idx)
